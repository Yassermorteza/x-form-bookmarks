...
form.addEventListener('submit', function (event) {
    ...
    if (!localStorage.storedSkills) {
        ...
        skillArray.push(skillsObject);
        var filteredSkills = skillArray.filter(function (el) {
            return !el.hasOwnProperty(selectedSkills);
        });
...
function display() {
    ...
    var outPut = storedValues.map(function (el) {
        var SkillName;
        for (var i = 0; i < skillCollection.length; i++) {
            skillCollection[i] === el[skillCollection[i]] ? SkillName = skillCollection[i] : false;
        }
    ...
    removeBtn.forEach(function (el, i) {
        el.addEventListener('click', function () {
            showSkills.removeChild(addedSkill[i]);
            var content = el.previousSibling.textContent;
            for (var _i = 0; _i < storedValues.length; _i++) {
                storedValues[_i][content] === content ? storedValues.splice(_i, 1) : false;
            }
